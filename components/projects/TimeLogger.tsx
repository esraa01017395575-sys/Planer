...
export const TimeLogger: React.FC<TimeLoggerProps> = ({ project, onLogTime }) => {
  ...
  const handleSave = () => {
    ...
    if (calculatedMinutes <= 0) return alert("الرجاء إدخال وقت صحيح / Please provide a valid duration.");

    onLogTime({
      title: sessionTitle || `سجل عمل مسبق تتبع ${project.title}`,
      description: `تسجيل وقت يدوي لقضاء ${calculatedMinutes} دقيقة`,
    ...
    <div className="glass-card p-6 space-y-5">
      <div>
        <h3 className="font-display font-bold text-base text-text-primary flex items-center gap-2">
          ...
          إدخال وقت بأثر رجعي / Log Time Manually
        </h3>
        <p className="text-xs text-text-secondary mt-1">سجل الساعات الجاهزة المكتملة دون استخدام مؤقت</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Hours & Minutes */}
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">الساعات / Hours</label>
          <input
            type="number"
            min="0"
            max="24"
        ...
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">الدقائق / Minutes</label>
          <input
            type="number"
            min="0"
            max="59"
        ...
        {/* Date Selector */}
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">التاريخ / Date Worked</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Title */}
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">موضوع الإنجاز / Task Title</label>
          <input
            type="text"
            value={sessionTitle}
            onChange={(e) => setSessionTitle(e.target.value)}
            placeholder="مثال: واجهة الصفحة الرئيسية..."
            className="w-full h-11 px-4 rounded-xl bg-bg-secondary border border-border/15 text-sm"
          />
        </div>
        {/* Mood */}
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">الحالة النفسية والمزاج / Mood Score</label>
          <select
            value={mood}
          ...
          >
            <option value="productive">🔥 منجز ومتحمس / Highly Productive</option>
            <option value="stuck">⚠️ عالق أو مشتت / Felt Stuck</option>
            <option value="breakthrough">✨ حدث طفرة أو إلهام / Breakthrough Ideas</option>
          </select>
        </div>
      </div>

      <div>
        <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">ملاحظات ودروس مستفادة / Key Session Notes</label>
        <textarea
          value={sessionNotes}
          onChange={(e) => setSessionNotes(e.target.value)}
          placeholder="شرح مختصر لما قمت به وأي تحديات تخطيتها..."
          rows={2}
      ...
      {allSuggestedTasks.length > 0 && (
        <div>
          <label className="block text-xs font-bold text-text-secondary uppercase mb-1.5">ربطه بالمهام المطلوبة / Link Completed Tasks</label>
      ...
      >
        ...
        <span>تسجيل الساعات / Log retrospect hours</span>
      </button>
    </div>
  );
};
