...
export function PomodoroTimer({
  taskId, taskTitle, pomodoroType = 'classic', onClose, floating = false, onMinimize, autoStart = false, customFocusMinutes
}: Props) {
  const cfg = CONFIGS[pomodoroType] || CONFIGS['classic'];
  const focusMinutes = customFocusMinutes || cfg.focus;
  
  const [phase, setPhase] = useState<Phase>('focus');
  const [sessionCount, setSessionCount] = useState(0);
  const [totalSecs, setTotalSecs] = useState(focusMinutes * 60);
  ...
  const handlePhaseEnd = async () => {
    ...
    if (phase === 'focus') {
      ...
      // Save to DB
      recordPomodoro({ task_id: taskId, duration_minutes: focusMinutes });
...
export function FloatingPomodoro({ taskId, taskTitle, pomodoroType, onClose, autoStart = false, customFocusMinutes }: Props) {
